...
light_signal_status = ceil(10^N_digits * rand(N_numbers,1));
...
if (0~=flag_try_move) 
    if (1==light_signal_status)
        player_position_internal = player_position_internal+1;
        fprintf(1,'Player is in a green light and steps forward to position: %.0d', player_position_internal);
    else 
        % Go back to the beginning!
        player_position_internal = 0;
        fprintf(1,'Player is in a red light and steps forward. Player is sent back to start position! Now at: %.0d', player_position_internal);
    end
end
...
%% Step 4 - Is it solved?
correct_answer = fcn_CodeX_calculateNameHash(sprintf('%.0d',student_number),'Winner!');
