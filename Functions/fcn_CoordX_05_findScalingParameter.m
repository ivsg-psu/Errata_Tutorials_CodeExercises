...
if nargin==nargin_lock_number
    ...
    if strcmp(Grader_input_code,date_lock_value)
        ...
        random_points_squared = (random_points(index_pairs(:,1),:) - random_points(index_pairs(:,2),:)).^2;
        random_points_distances_squared = sum(random_points_squared,2);

        noisy_points_squared = (noisy_points(index_pairs(:,1),:) - noisy_points(index_pairs(:,2),:)).^2;
        noisy_points_distances_squared = sum(noisy_points_squared,2);

        y = random_points_distances_squared.^0.5;
        x = noisy_points_distances_squared.^0.5;
        M = (y'*x)\(y'*y);
        S_new = 1/M;

        correct_answer = S_new;
