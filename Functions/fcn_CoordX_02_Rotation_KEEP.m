...
dependencies = 'fcn_CoordX_01_Translation, fcn_CoordX_02_Rotation';
...
this_fname = 'fcn_CoordX_02_Rotation';
...
if nargin==nargin_lock_number
    ...
    if strcmp(Grader_input_code,date_lock_value)
        ...
        T(1,1) = cos(theta);   %Transformation Matrix
        T(1,2) = -sin(theta);
        T(2,1) = sin(theta);
        T(2,2) = cos(theta);
        ...
        %max_x_after_rotation = max(moved_points(:,1));
        [~,max_x_Index_after_rotation] = max(moved_points(:,1));

        correct_answer = max_x_Index_after_rotation;
